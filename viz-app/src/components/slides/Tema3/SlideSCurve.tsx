...
import { Wallet, Trophy, Map, Info } from 'lucide-react';
import { Card } from '../../ui/Card';
import SlideContainer from '../../shared/SlideContainer';

const SlideSCurve = ({ autoPlay, onAudioComplete }: { autoPlay?: boolean; onAudioComplete?: () => void }) => {
    ...
    const ttsSteps = [
        {
            id: "intro",
            text: "La Curva S. Muestra la evolución acumulada de costes y trabajo a lo largo del tiempo. Su forma de 'S' es natural: inicio lento, ejecución rápida, cierre lento."
        },
        {
            id: "lines",
            text: "Líneas de Seguimiento. La Azul es tu plan (PV). La Verde es lo que realmente has hecho (EV). La Roja es lo que has gastado (AC)."
        },
        {
            id: "variances",
            text: "Análisis de Variaciones. Las diferencias verticales entre líneas nos dicen si vamos retrasados (SV) o si estamos perdiendo dinero (CV)."
        },
        {
            id: "conclusion",
            text: "Conclusión. Si la línea verde está por debajo de la azul, vamos lentos. Si la roja está por encima de la verde, estamos gastando de más."
        }
    ];

    return (
        <SlideContainer
            title="La Curva S (EVM)"
            rate={1.2}
            ttsSteps={ttsSteps}
            autoPlay={autoPlay}
            onStepChange={(id) => {
                ...
                if (id === 'intro') setTimeIndex(10);
                if (id === 'lines') setTimeIndex(40);
                if (id === 'variances') setTimeIndex(70);
                if (id === 'conclusion') setTimeIndex(90);
            }}
        >
