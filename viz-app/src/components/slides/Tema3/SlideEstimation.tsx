...
const SlideEstimation = ({ autoPlay, onAudioComplete }: { autoPlay?: boolean; onAudioComplete?: () => void }) => {
    ...
    const ttsSteps = [
        {
            id: "intro",
            text: "Estimación de Duración. Para definir el Cronograma, primero debemos estimar cuánto durarán las actividades."
        },
        {
            id: "pert",
            text: "Laboratorio PERT. Esta técnica de tres valores (Optimista, Probable, Pesimista) nos permite calcular una duración esperada ponderada y medir el riesgo."
        },
        {
            id: "others",
            text: "Otras Técnicas. Usamos el Juicio de Expertos, la Estimación Análoga por comparación histórica, y la Paramétrica basada en estadísticas."
        }
    ];

    return (
        <SlideContainer
            title="Estimación de Duración"
            rate={1.2}
            ttsSteps={ttsSteps}
            autoPlay={autoPlay}
            onStepChange={(id) => {
        ...
        >
            <div className={`space-y-8 animate-fade-in ${activeSection ? 'transition-opacity duration-500' : ''}`}>

                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">

                    {/* 1. PERT Laboratory */}
                    <div className={`bg-white dark:bg-slate-800 rounded-xl shadow-lg border-t-4 border-blue-500 p-6 transition-all duration-500 ${activeSection === 'pert' ? 'ring-4 ring-blue-400 scale-[1.02]' : ''}`}>
                    ...
                    {/* 2. Other Techniques */}
                    <div className={`space-y-4 transition-all duration-500 ${activeSection === 'others' ? 'ring-4 ring-orange-400 rounded-xl p-2 bg-orange-50 dark:bg-orange-900/10' : ''}`}>
                        {/* Expert Judgment */}
