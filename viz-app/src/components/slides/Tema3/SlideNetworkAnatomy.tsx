...
const SlideNetworkAnatomy = ({ autoPlay, onAudioComplete }: { autoPlay?: boolean; onAudioComplete?: () => void }) => {
    ...
    const ttsSteps = [
        {
            id: "intro",
            text: "Anatomía de la Red. El Diagrama de Precedencia o PDM nos permite visualizar la secuencia lógica del trabajo."
        },
        {
            id: "fs",
            text: "Relaciones Lógicas. La relación Fin-Inicio es la estándar: una tarea desbloquea a la siguiente. Pero también existen Inicio-Inicio y Fin-Fin para trabajos paralelos."
        },
        {
            id: "adjustment",
            text: "Ajustes de Tiempo. Usamos 'Lead' o Adelanto para acelerar solapando tareas, y 'Lag' o Retraso para imponer esperas técnicas obligatorias sin consumir recursos."
        }
    ];

    return (
        <SlideContainer
            title="Anatomía de la Red (PDM)"
            rate={1.2}
            ttsSteps={ttsSteps}
            autoPlay={autoPlay}
            onStepChange={(id) => {
                ...
                if (id === 'fs') playFs();
                if (id === 'adjustment') {
                    // Slight delay to ensure element is visible before animating
                    setTimeout(playLead, 500);
                }
            }}
        >
            <div className={`space-y-8 animate-in fade-in duration-500 ${activeSection ? 'transition-all duration-500' : ''}`}>

                {/* SECTION 1: RELACIONES */}
                <section className={`space-y-6 transition-all duration-500 ${activeSection === 'fs' ? 'ring-4 ring-blue-400 p-2 rounded-xl bg-blue-50 dark:bg-blue-900/10' : ''}`}>
                ...
                <section className={`space-y-6 transition-all duration-500 ${activeSection === 'adjustment' ? 'ring-4 ring-purple-400 p-2 rounded-xl bg-purple-50 dark:bg-purple-900/10' : ''}`}>
                    <div className="flex items-center gap-3">
